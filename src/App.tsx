...
export default function App() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.error(error);
      showToast(
        'Gagal memproses transaksi ke Cloud! Pastikan Anda sudah "Login dengan Google" dan terhubung internet.',
      );
    } finally {
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      ...
      console.error(error);
      showToast(
        'Gagal menyimpan ke Cloud! Pastikan Anda sudah "Login dengan Google" untuk hak akses database.',
      );
    }
  };
  ...
  return (
    ...
    >
      {/* Sidebar Navigation */}
      <aside
        className={`hidden md:flex w-24 border-r flex-col flex-shrink-0 ${
          darkMode
      ...
      <main className="flex-1 overflow-hidden flex flex-col">
        ...
        {view === "stock" && (
          <div className="p-4 md:p-6 xl:p-8 overflow-y-auto flex-1 animate-in fade-in duration-300">
            ...
            <div
              className={`rounded-3xl borded overflow-hidden transition-all ${
                darkMode
                  ? `bg-slate-900 border-slate-800`
            ...
            {isAddingProduct && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                <div
                  className={`w-full max-w-lg rounded-3xl p-4 md:p-6 xl:p-8 animate-in zoom-in-95 duration-200 ${
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex space-x-4 mt-2 text-[10px] font-bold uppercase text-slate-400">
                      <span>
                        Tunai: Rp {t.paidAmount?.toLocaleString("id-ID")}
                      </span>
                      <span>
                        Kembali: Rp {t.changeAmount?.toLocaleString("id-ID")}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-lg p-8 animate-in zoom-in-95 duration-200 relative overflow-hidden">
            ...
            <div className="border-t border-dashed border-slate-200 py-4 space-y-1">
              ...
              <div className="flex justify-between text-xs text-slate-500">
                <span>Tunai</span>
                <span>
                  {lastTransaction.paidAmount.toLocaleString("id-ID")}
                </span>
              </div>
              <div className="flex justify-between text-xs text-slate-500">
                <span>Kembali</span>
                <span>
                  {lastTransaction.changeAmount.toLocaleString("id-ID")}
                </span>
              </div>
            </div>
